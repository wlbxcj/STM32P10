...
#ifdef RF_L1_RECDBG//emv test switch


#define	PiccOpen			picc_open
#define	PiccClose			picc_close
#define	PiccCmdExchange		        picc_cmdexchange
#define	PiccDetect			picc_detect
#define	PiccIsoCommand		        picc_isocommand
#define	PiccRemove			picc_remove
#define	PiccSetup			picc_setup
...
//uchar gaucCmdErr[256];  //2010/01/11 gplian
//uchar gaucDetErr[256];  //2010/01/11 gplian
...
uchar gl_aucRecData[1024*1];   //2010.01.11 gplian,现没用，缩小.01.15不能缩小(s_RF_vHALTA在用)
...
extern void debug_printf(unsigned char col,unsigned char row,unsigned char mode,char *str,...);

	#if (TERM_TYPE == TERM_KF322)
		#define ScrPrint	debug_printf
		extern unsigned char kbhit(void);
		extern unsigned char getkey(void);
		void PiccLight(uchar index,uchar mode)
		{
			if( mode )//light on
			{
				if( index& PICC_LED_RED )	ledred_on();
				if( index& PICC_LED_YELLOW )	ledorange_on();
				if( index& PICC_LED_BLUE || index& PICC_LED_GREEN )	ledgreen_on();
			}
			else
			{
				if( index& PICC_LED_RED )	ledred_off();
				if( index& PICC_LED_YELLOW )	ledorange_off();
				if( index& PICC_LED_BLUE || index& PICC_LED_GREEN)	ledgreen_off();
			}
		}
...
uchar test_GetNewVal(uchar ucMin, uchar ucMax, uchar ucRst, uchar ucCur, uchar *pucVal)
{
	...
	while(1)
	{
		ScrPrint(0,1,0,"Range:%02X~%02XH,Rst:%02XH", ucMin, ucMax, ucRst);  
		ScrPrint(0,2,0,"Cur: %02X  ", ucVal);
		ScrPrint(0,3,0,"New: %02X  ", ucTemp);

		ScrPrint(0,5,0,"STEP: F1-1 F2-5 \nF3-10 F4-Quit ");
		ucKey = getkey();
		if (ucKey == RF_KEY_F1) 
		{
			ucStep = 1;
...
void test_SetParam(void)
{
	...
	while(1)
	{
		ScrPrint(0,0,0x40,"  PCD Param Set");
		ScrPrint(0,1,0,"F1-A_CwConduct"); // CWGsPREG ADD:28H,DEF VAL:0x3F
		ScrPrint(0,2,0,"F2-B_ModConduct "); // GsNOnREG bit7~4 ADD:27H,DEF VAL:0xF0 低4位数据将被清除
		ScrPrint(0,3,0,"F3-RxThrsHld"); // ModGsPREG bit5~0 add 29h, DEF val:0x11 
		ScrPrint(0,4,0,"F4-Quit ");
		...
		case RF_KEY_F1:
			...
			ScrPrint(0,0,0," A_CwConduct:%02X  ", ucTemp);			
		...
		case RF_KEY_F2:
			...
			ScrPrint(0,0,0," B_ModConduct:%02X  ", ucTemp);			
			if(!test_GetNewVal(ucMin, ucMax, ucRst, ucTemp, &ucVal))
				ucTemp = ucVal;	 
		...
		case RF_KEY_F3:
			...
			ucMin = 0x00;
			ucRst = 0x88; 
			ucTemp = my_c_para.card_RxThreshold_val;
			ScrPrint(0,0,0," RxThreshold:%02X  ", ucTemp); 		
			if(!test_GetNewVal(ucMin, ucMax, ucRst, ucTemp, &ucVal))
				ucTemp = ucVal; 
			my_c_para.card_RxThreshold_val = ucTemp;
			ScrCls();
			break;
		...
		my_c_para.card_RxThreshold_w=1;
...
void test_LoopABTest(void)
{
	...
	char gaucCmdErr[256];  //2010/01/11 gplian
	char gaucDetErr[256];  //2010/01/11 gplian
	...
	while(1)
	{
		...
		case RF_KEY_F1:
			...
			while(1)
			{
				...
				s_vRFWriteReg(1, RF_REG_MODWIDTH, buf);
				iRet = s_vRFReadReg(1, RF_REG_MODWIDTH, buftwo);
...
//					DelayMs(1000);
		...
		case RF_KEY_F2:
			...
			while(1)
			{
				...
				s_vRFWriteReg(1, RF_REG_MODWIDTH, buf);
				s_vRFReadReg(1, RF_REG_MODWIDTH, buftwo);
...
{
  uchar i;
  debug_printf(0,0,0,"Rf Test For All Fun.[V1.10]");
  debug_printf(0,0,0,"1-QpbocTest,2-ApiTest,3-SpiTest");
  i=getkey();
  if(i=='1')
  MifreQpbocTest();
  if(i=='2')
