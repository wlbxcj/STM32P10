...
FLASH_Status FLASH_ProgramHalfWord(u32 Address, u16 Data)
{
  ...
  /* Wait for last operation to be completed */
  //status = FLASH_WaitForLastOperation(ProgramTimeout);
  //status = FLASH_WaitForLastOperation(ProgramTimeout*5000); //13/05/31 gplian
  status = FLASH_WaitForLastOperation(ProgramTimeout); //13/06/02 gplian
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    *(vu16*)Address = Data;
    /* Wait for last operation to be completed */
    //status = FLASH_WaitForLastOperation(ProgramTimeout);
    //status = FLASH_WaitForLastOperation(ProgramTimeout*5000); //13/05/31
    status = FLASH_WaitForLastOperation(ProgramTimeout); //13/06/02
...
{ 
  ...
  if(status == FLASH_ERROR_PG)
    trace_debug_printf("FLASH_ERROR_PG\n");
  
  /* Wait for a Flash operation to complete or a TIMEOUT to occur */
  //while((status == FLASH_BUSY) && (Timeout != 0x00))
  
  while( ((status == FLASH_BUSY) || (status == FLASH_ERROR_PG))  && (Timeout != 0x00)) //13/05/31
  {
    delay();
    status = FLASH_GetStatus();
    Timeout--;
  ...
  {
    //liantest
    trace_debug_printf("Timeout FLASH_GetStatus[%d]\n",status);
