...
if(bFlag)
    ...
    if(fd<0)
    {
        ...
        //13/07/11
#if 0    //not init
        for(i=0;i<(PCI_BACK_ADDR)/1024;i++)
...
{
    int iret=0;
    uchar   mkeybuf[24], keybuf[16], buf[16], mkeylen;
    uchar use_gm;
	...
	switch(mkey_part) 
	{
		case 0x00:
			if (use_gm) 
			{
				memcpy(buf, indata, 16);
				iret=Gm_Sm4(buf,16,buf,mkeybuf,mode);
				memcpy(outdata, buf, 16);
			}
			else
                        {
				memcpy(buf, indata, 8);
...
int Lib_PciWriteWORK_MKey(uchar key_no,uchar key_len,uchar *key_data,uchar *key_crc,uchar mode, uchar mkey_no)
{
	...
	switch(mode)
    {
		...
		case 0x81 | MODE_GM:
			...
			if (mode & MODE_GM)	
			{
				encrypt = (mode & 0x80) ? 0 : 1;
				iret = Gm_Sm4(keybuf,key_len,keybuf, mkeybuf, encrypt);
			}
			else 
...
int Lib_PciWriteWorkKey(uchar key_no,uchar key_len,uchar *key_data, uchar *key_crc,uchar mode, uchar mkey_no)
{
	...
	{
			 encrypt = (mode & 0x80) ? 0 : 1;
			 iret = Gm_Sm4(pinkeybuf,key_len,pinkeybuf, mkeybuf, encrypt);
	}
	else         
          iret=DES_TDES(mkeybuf,mkeylen,pinkeybuf,key_len,mode);
...
void SM4_Mac1(uchar *key,uchar *mdat,ushort length)
{
	ushort i,j,l;
	...
	if(i)
	{
		 memset(&mdat[length],0x00,16-i);
		 l ++;
	}
	if(l<=1)
	{
		 Gm_Sm4(mdat, 16, mdat, key, 1);
		 return;
	}
	for(i=1;i<l;i++)
	{
		 for(j=0;j<16;j++) mdat[j] ^= mdat[j+16*i];
	} 
	Gm_Sm4(mdat, 16, mdat, key, 1);
}

void SM4_Mac2(uchar *key,uchar *mdat,ushort length)
{
	ushort i,j,l;
	l = length/16;
	i = length%16;
...
void SM4_Mac3(uchar *key,uchar *mdat,ushort length)
{
	ushort i,j,l;
	l = length/16;
	i = length%16;
...
int SM4_GetMac(BYTE mackey_n,WORD inlen,BYTE *indata,BYTE *macout,BYTE mode)
{
	...
	if(mode==1)
	{
		SM4_Mac1(mackeybuf, datain, inlen);	
	}
	else if(mode==2)
	{		 
		SM4_Mac2(mackeybuf, datain, inlen);	
	}	
	else
	{		 
		SM4_Mac3(mackeybuf, datain, inlen);	
	}
	memcpy(macout,datain,16);
...
int Lib_PciGetMac(BYTE mackey_n,WORD inlen,BYTE *indata,BYTE *macout,BYTE mode)
{
	...
	if (mode & MODE_GM)
		ret = SM4_GetMac(mackey_n, inlen, indata, macout, mode & 0x0F);
	else
		ret = Des_GetMac(mackey_n, inlen, indata, macout, mode & 0x0F);
