...
void as3911Transmit(const u8 *message, u32 messageLength, AS3911RequestFlags_t requestFlags)
{
    ...
    do
    {
        ...
        s_UartPrint("tirqStatus=%d\n",irqStatus);
		//LOG("as3911Transmit is go");
        ...
        {
        	//s_UartPrint("end transmission.");
                 return;
                  //delay_ms(1); //sxl
        }
    } while (0 != irqStatus);
       s_UartPrint("iirqStatus=%d\n",irqStatus);
	//s_UartPrint("error transmission.");
     //delay_ms(1); //sxl
...
s16 as3911Receive(u8 *response, u32 maxResponseLength, u32 *responseLength)
{
	...
	*responseLength = 0;
    ...
    do
    {
        ...
        //s_UartPrint("irqStatus1 = %d\r\n", irqStatus);//sxl
        //delay_ms(1); //sxll
        if (irqStatus & AS3911_IRQ_MASK_WL)
        {
           //s_UartPrint("AS3911_IRQ_MASK_WL\r\n"); //sxl
            /* FIFO water level interrupt. */
            u8 auxDisplay = 0;
            u8 fifoStatus[2];
            u8 numBytesInFifo = 0;
            ...
            while ((auxDisplay & AS3911_AUX_DISPLAY_RXON_BIT) && (numBytesInFifo > 0))
            {
                ...
                {
                    s_UartPrint("numBytesReceived[%d]fifoReadSize[%d]maxResponseLength[%d]\r\n",numBytesReceived,fifoReadSize,maxResponseLength);//gplian
        ...
        if (irqStatus & AS3911_IRQ_MASK_RXS)
        {
            ...
            receiving = TRUE;
            //s_UartPrint("AS3911_IRQ_MASK_RXS"); //sxl
        }
        if (irqStatus & AS3911_IRQ_MASK_NRE)
        {
            ...
            if (!receiving)
			{
                ...
                nrtExpired = TRUE;
            //s_UartPrint("AS3911_IRQ_MASK_NRE"); //sxl
        }
        if (irqStatus & AS3911_IRQ_MASK_RXE)
        {
            ...
            //s_UartPrint("AS3911_IRQ_MASK_RXE"); //sxl
            //s_UartPrint("errorIrqStatus = %d\r\n", errorIrqStatus);//sxl
           // delay_ms(1); //sxl
//LOG("as3911GetInterrupts fifoStatus[0]=0x%x, fifoStatus[1]=0x%x", fifoStatus[0], fifoStatus[1]);
//s_UartPrint("as3911GetInterrupts fifoStatus[0]=0x%x, fifoStatus[1]=0x%x\r\n", fifoStatus[0], fifoStatus[1]);//gplian
	...
	s_UartPrint("irqStatus=%d\nAS3911_INTERNAL_ERROR=%d\n",irqStatus,AS3911_INTERNAL_ERROR);
    return AS3911_INTERNAL_ERROR;
}
